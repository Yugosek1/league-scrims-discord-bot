...
async def edit_list():
   ...
   if mypost:      
      ...
      for i in range(len(mypost)):
         embed1.add_field(name=str(i+1), value=
         f'`チーム名`{mypost[i][1]}\n`対戦開始日時`{mypost[i][2]}`平均レート`{mypost[i][7]}\n`試合数`{mypost[i][4]}`コメント`{mypost[i][5]}\n`連絡先`<@{mypost[i][0]}>`post_ID`{mypost[i][6]}'
         , inline=False)
...
async def post_mylist(message):
   # mypost = c.execute('SELECT user_id, teamname, date_and_time, tier_average, matches, comments, rowid FROM database WHERE user_id =?',[message.author.id]).fetchall()
   mypost = c.execute('SELECT user_id, teamname, strftime("%m月%d日 %H時%M分",date_and_time), tier_average, matches, comments, database.rowid, tier FROM database join tier_list using(tier_average) WHERE user_id =? order by date_and_time asc limit 10',[message.author.id]).fetchall()
