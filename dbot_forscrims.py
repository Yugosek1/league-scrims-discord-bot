...
async def post_list():
   ...
   if mypost:
      for i in range(len(mypost)):
         ...
         embed.add_field(name="対戦開始日時", value=mypost[i][2].strftime('%m月%d日 %H時%M分'), inline=True)
      ...
      for i in range(len(mypost)):
         ...
         "`チーム名`: "+mypost[i][1]+"\n"+
         " `対戦開始日時`: "+mypost[i][2].strftime('%m月%d日 %H時%M分')+
         " `平均レート`: "+mypost[i][7]+"\n"+
         " `試合数`: "+mypost[i][4]+
         " `コメント`: "+mypost[i][5]+"\n"+
         " `連絡先`: "+"<@"+str(mypost[i][0])+">"
...
async def post_refresh():
   cur.execute('DELETE from database WHERE date_and_time <= current_date - 1;')
   conn.commit()
